...
// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //
namespace Foam {
    const scalar treeBoundBox::great(GREAT);
    ...
    const treeBoundBox treeBoundBox::invertedBox
    (
        vector(GREAT, GREAT, GREAT),
        vector(-GREAT, -GREAT, -GREAT)
    );
    ...
    const FixedList<vector, 6> treeBoundBox::faceNormals
    (
        calcFaceNormals()
    );
    ...
    edgeList treeBoundBox::calcEdges(const label edgesArray[12][2])
    {
        edgeList edges(12);
        forAll(edges, edgeI)
        {
    ...
    FixedList<vector, 6> treeBoundBox::calcFaceNormals()
    {
    ...
    treeBoundBox::treeBoundBox(const UList<point>& points) : boundBox(points, false)
    {
        if (points.empty())
        {
            WarningInFunction
