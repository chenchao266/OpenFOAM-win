...
void cyclicPolyPatch::getCentresAndAnchors
(
...
vector cyclicPolyPatch::findFaceMaxRadius
(
...
const word& cyclicPolyPatch::neighbPatchName() const
{
...
label cyclicPolyPatch::neighbPatchID() const
{
    if (neighbPatchID_ == -1)
    {
...
void cyclicPolyPatch::transformPosition(pointField& l) const
{
    if (!parallel())
    {
        if (transform() == ROTATIONAL)
...
void cyclicPolyPatch::transformPosition(point& l, const label facei) const
{
    if (!parallel())
    {
        const tensor& T =
...
void cyclicPolyPatch::initGeometry(PstreamBuffers& pBufs)
{
...
void cyclicPolyPatch::calcGeometry(PstreamBuffers& pBufs)
{
    calcGeometry
    (
        *this,
...
void cyclicPolyPatch::initMovePoints
(
...
void cyclicPolyPatch::movePoints
(
    PstreamBuffers& pBufs,
    const pointField& p
)
...
void cyclicPolyPatch::initUpdateMesh(PstreamBuffers& pBufs)
{
    polyPatch::initUpdateMesh(pBufs);
}


void cyclicPolyPatch::updateMesh(PstreamBuffers& pBufs)
{
...
void cyclicPolyPatch::write(Ostream& os) const
{
