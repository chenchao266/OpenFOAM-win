...
// a hexahedral cell shape model and a hexCell
 ...
 namespace Foam{
const label hexCell::modelFaces_[6][4] =
{
    {0, 4, 7, 3},  // x-min
    {1, 2, 6, 5},  // x-max
    {0, 1, 5, 4},  // y-min
...
const label hexCell::modelEdges_[12][2] =
{
    {0, 1},  // x-direction
    {3, 2},
    {7, 6},
...
const faceList& hexCell::modelFaces()
{
    static std::unique_ptr<faceList> ptr(nullptr);

    if (!ptr)
    {
        ptr.reset(new faceList(hexCell::nFaces(), face(4)));
...
const edgeList& hexCell::modelEdges()
{
    static std::unique_ptr<edgeList> ptr(nullptr);

    if (!ptr)
    {
        ptr.reset(new edgeList(hexCell::nEdges()));
...
faceList hexCell::faces() const
{
    faceList theFaces(hexCell::nFaces(), face(4));
...
edgeList hexCell::edges() const
{
    edgeList theEdges(hexCell::nEdges());
...
cellShape hexCell::shape(const bool doCollapse) const
{
