...
label wedgeMatcher::faceHashValue() const
{
    return 2*3 + 4*4;
}


bool wedgeMatcher::faceSizeMatch
(
    const faceList& faces,
    const labelList& myFaces
) const
...
bool wedgeMatcher::isA(const primitiveMesh& mesh, const label celli)
{
    return matchShape
    (
        true,
...
bool wedgeMatcher::isA(const faceList& faces)
{
