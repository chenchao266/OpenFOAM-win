...
label prismMatcher::faceHashValue() const
{
    return 2*3 + 4*4;
}


bool prismMatcher::faceSizeMatch
(
    const faceList& faces,
    const labelList& myFaces
) const
...
bool prismMatcher::isA(const primitiveMesh& mesh, const label celli)
{
    return matchShape
    (
        true,
...
bool prismMatcher::isA(const faceList& faces)
{
