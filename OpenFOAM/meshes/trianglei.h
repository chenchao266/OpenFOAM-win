...
namespace Foam {
    ...
    template<class Point, class PointRef>
    inline Point triangle<Point, PointRef>::randomPoint
    (
        cachedRandom& rndGen
    ) const
    {
    ...
    ) const
    {
        ...
        bool hit;
        {
            ...
            pointHit fastInter = intersect(p, q1, intersection::FULL_RAY);
            hit = fastInter.hit();
    ...
    // Moeller/Trumbore.
    template<class Point, class PointRef>
    inline pointHit triangle<Point, PointRef>::intersect
    (
        const point& orig,
        const vector& dir,
        const intersection::algorithm alg,
    ...
    ) const
    {
        ...
        // Initialise to miss
        pointHit intersection1(false, Zero, GREAT, false);
        ...
        intersection1.setHit();
        intersection1.setPoint(a_ + u * edge1 + v * edge2);
        intersection1.setDistance(t);
    ...
    ) const
    {
        ...
        pointHit triInfo
        (
            triangle<Point, PointRef>::intersect
            (
                ln.start(),
                q,
