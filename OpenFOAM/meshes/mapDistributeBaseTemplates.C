...
void mapDistributeBase::flipAndCombine
(
...
T mapDistributeBase::accessAndFlip
(
    const UList<T>& fld,
    const label index,
    const bool hasFlip,
...
template<class T>
void mapDistributeBase::send(PstreamBuffers& pBufs, const List<T>& field)
const
{
    // Stream data into buffer
...
template<class T>
void mapDistributeBase::receive(PstreamBuffers& pBufs, List<T>& field)
const
{
    // Consume
...
template<class T>
void mapDistributeBase::reverseDistribute
(
    const label constructSize,
    List<T>& fld,
    const int tag
