...
// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //
 ...
 namespace Foam{
const char* const face::typeName = "face";


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

tmp<vectorField>
face::calcEdgeVectors(const UList<point>& points) const
{
...
int face::compare(const face& a, const face& b)
{
...
bool face::sameVertices(const face& a, const face& b)
{
    const label sizeA = a.size();
    const label sizeB = b.size();
...
unsigned face::symmhash_code(const UList<label>& f, unsigned seed)
{
    ::Foam::Hash<label> op;
...
point face::centre(const UList<point>& points) const
{
...
vector face::areaNormal(const UList<point>& p) const
{
    const label nPoints = size();
...
int face::edgeDirection(const ::Foam::edge& e) const
{
...
label face::nTriangles(const UList<point>&) const
{
    return nTriangles();
}
...
label face::longestEdge(const UList<point>& pts) const
{
    const labelList& verts = *this;
    const label nVerts = verts.size();
