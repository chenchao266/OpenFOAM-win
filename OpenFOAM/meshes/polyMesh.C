...
autoPtr<labelIOList> polyMesh::readTetBasePtIs() const
{
    IOobject io
    (
        "tetBasePtIs",
...
const Vector<label>& polyMesh::geometricD() const
{
    if (geometricD_.x() == 0)
    {
        calcDirections();
...
const Vector<label>& polyMesh::solutionD() const
{
    if (solutionD_.x() == 0)
    {
        calcDirections();
...
bool polyMesh::upToDatePoints(const regIOobject& io) const
{
    return io.upToDate(points_);
}


void polyMesh::setUpToDatePoints(regIOobject& io) const
{
...
const labelList& polyMesh::faceOwner() const
{
    return owner_;
}


const labelList& polyMesh::faceNeighbour() const
{
    return neighbour_;
}
...
void polyMesh::removeFiles() const
{
    removeFiles(instance());
}


void polyMesh::findCellFacePt
(
    const point& p,
    label& celli,
    label& tetFacei,
