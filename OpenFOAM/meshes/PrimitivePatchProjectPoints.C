...
List<objectHit>
PrimitivePatch<FaceList, PointField>::projectPoints
(
...
template<class ToPatch>
List<objectHit>
PrimitivePatch<FaceList, PointField>::projectFaceCentres
(
    const ToPatch& targetPatch,
    const Field
    <
        ...
        if (doNSquaredSearch || !foundEligible)
        {
            ...
            forAll(masterFaces, facei)
            {
                ...
                else if (curHit.eligibleMiss())
                {
                    // Calculate min distance
                    scalar missDist =
                        mag(curHit.missPoint() - curFaceCentre);
