...
# Example
 ...
 namespace Foam{
# rule_method_component(sfield, tfield, CMPT_XX, tensor::XX)
#
# sfield (lhs) ::= tfield (a) DOT method_call CMPT_XX LPAREN RPAREN. [DOT]
# {
#     lhs = make_tmp(a)->component(tensor::XX).ptr();
# }
#------------------------------------------------------------------------------
...
[$1 (lhs) ::= $2 (a) DOT DIAG LPAREN RPAREN. _lemon_precedence(DOT)
{
    ...
    unzipDiag(field_read_access(make_obj(a)), field_write_access(*lhs));
}]
)
...
#
# Example
# rule_tensor_unzipRow(vfield, tfield, CMPT_CX, vector::X)
#
# vfield (lhs) ::= tfield (a) DOT CMPT_CX LPAREN RPAREN. [DOT]
# {
#     lhs = _new_vfield();
#     unzipCol(make_obj(a), vector::X, *lhs);
# }
#------------------------------------------------------------------------------
...
#
# Example
# rule_tensor_unzipRow(vfield, tfield, CMPT_X, vector::X)
#
# vfield (lhs) ::= tfield (a) DOT CMPT_X LPAREN RPAREN. [DOT]
# {
#     lhs = _new_vfield();
#     unzipRow(make_obj(a), vector::X, *lhs);
# }
#------------------------------------------------------------------------------
