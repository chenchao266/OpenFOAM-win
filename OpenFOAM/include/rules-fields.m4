...
# {
#     lhs = driver->getVolField<scalar>(make_obj(name.name_)).ptr();
# }
#------------------------------------------------------------------------------
...
# sfield (lhs) ::= svalue (a) .
# {
#     lhs = driver->newField<scalar>(make_obj(a)).ptr();
# }
#
# psfield (lhs) ::= POINT_EXPR LPAREN svalue (a) RPAREN .
# {
#     lhs = driver->newPointField<scalar>(make_obj(a)).ptr();
# }
#------------------------------------------------------------------------------
...
#
# Example
# rule_inplace_unary(vfield, MIN, min)
#
...
#
# Example
# rule_unary_func(sfield, vfield, MAG, mag)
#
# sfield (lhs) ::= MAG LPAREN vfield (a) RPAREN .
# {
#     lhs = mag(make_tmp(a)).ptr();
# }
#------------------------------------------------------------------------------
...
#
# Example
# rule_binary_func(sfield, sfield, sfield, POW, pow)
#
# sfield (lhs) ::= POW LPAREN sfield (a) COMMA sfield (b) RPAREN .
# {
#     /// checkSizes("$5 (a, b)", a, b);
#     lhs = pow(make_tmp(a), make_tmp(b)).ptr();
# }
#------------------------------------------------------------------------------
...
#
# Example
# rule_const_multiply(sfield, sfield, degToRad(), DEG_TO_RAD)
#
# sfield(lhs) ::= DEG_TO_RAD LPAREN sfield(a) RPAREN .
# {
#     lhs = ((degToRad()) * make_tmp(a)).ptr();
# }
#------------------------------------------------------------------------------
...
#
# Example
# rule_scalar_divide(vfield, vfield, sfield, vector)
#
...
[$1 (lhs) ::= $2 (a) DIVIDE $3 (b) .
{
    ...
    (
        *lhs,
        make_obj(a),
        make_obj(b),
        scalarDivideOp<ifelse($4,[], _value_type_$1, $4)>()
    );
}]
)
...
#     expressions::exprDriverOps::swakModuloOp()
#------------------------------------------------------------------------------
...
#
# Example
# rule_unary_assign(sfield, sfield, FLOOR, floorOp<scalar>())
#
#------------------------------------------------------------------------------
...
{
    lhs = _new_$1();
    FieldOps::assign(*lhs, make_obj(a), $4);
}]
)
...
#
# Example
# rule_binary_assign(sfield, sfield, sfield, HYPOT, hypot)
#
#------------------------------------------------------------------------------
...
{
    lhs = _new_$1();
    FieldOps::assign(*lhs, make_obj(a), make_obj(b), $5);
}]
)
...
# {
#     lhs = driver->patchNormalField<scalar>(make_obj(name.name_)).ptr();
# }
#
#------------------------------------------------------------------------------
