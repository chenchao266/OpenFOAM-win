...
# Description
 ...
 namespace Foam{
#     Logic rules, using bool or scalar for its storage.
#
#     `rule_logical_and'
#     `rule_logical_or'
#     `rule_logical_negate'
...
undefine([_logic_type_])dnl     # Eg, bool, scalar
undefine([_logic_true_])dnl     # Eg, true, scalar(1)
undefine([_logic_false_])dnl    # Eg, false, scalar(0)
...
#
# rule_logical_and(lfield, scalar)
#
...
# Example - using boolField for selections
#
# rule_ternary_op(vfield, lfield, vector)
#
# vfield (lhs) ::= lfield(cond) QUESTION vfield (a) COLON vfield (b) .
# {
#     lhs = a;
#
#     FieldOps::ternarySelect<vector>
#     (
#         *lhs, make_obj(cond), *a, make_obj(b)
#     );
# }
...
#     lhs = a;
#
#     FieldOps::ternarySelect<vector, scalar>
#     (
#         *lhs, make_obj(cond), *a, make_obj(b),
#         expressions::boolOp<scalar>()
#     );
# }
#------------------------------------------------------------------------------
...
[$1 (lhs) ::= $2 (cond) QUESTION $1 (a) COLON $1 (b) .
{
    ...
    (
        *lhs, make_obj(cond), *a, make_obj(b),
        expressions::boolOp<_logic_type_>()
    );])
}]
)
...
#
# Example
#     rule_cast_logical(lfield, sfield, scalar)
#------------------------------------------------------------------------------
...
#
# Example
# rule_binary_logical_op(lfield, vfield, vector, greaterOp, GREATER, >)
#
# lfield (lhs) ::= vfield (a) GREATER vfield (b) .
# {
#    /// checkSizes("(a $6 b"), a, b);
#    lhs = driver->newVolField<scalar>().ptr();
#
#    FieldOps::assign
#    (*lhs, make_obj(a), make_obj(b), greaterOp<vector>());
# }
#------------------------------------------------------------------------------
...
{
    lhs = _new_$1();
    FieldOps::assign(*lhs, make_obj(a), make_obj(b), $4<$3>());
}]
)
...
[define([_logic_type_], [scalar])dnl
define([_logic_true_], [scalar(1)])dnl
define([_logic_false_], [scalar(0)])]
)

# Define now as using scalar for the logic
use_scalar_logic()
