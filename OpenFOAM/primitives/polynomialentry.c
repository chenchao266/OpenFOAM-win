...
// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //
 ...
 namespace Foam{
template<class Type>
void Function1Types::Polynomial<Type>::checkCoefficients()
{
    if (coeffs_.empty())
    {
...
template<class Type>
Function1Types::Polynomial<Type>::Polynomial
(
...
template<class Type>
Function1Types::Polynomial<Type>::Polynomial(const Polynomial& poly)
:
    Function1<Type>(poly),
...
template<class Type>
void Function1Types::Polynomial<Type>::userTimeToTime(const Time& t)
{
    forAll(coeffs_, i)
    {
...
template<class Type>
Type Function1Types::Polynomial<Type>::value(const scalar x) const
{
    Type y(Zero);
    forAll(coeffs_, i)
    {
        ...
        (
            coeffs_[i].first(),
            cmptPow(pTraits<Type>::one_*x, coeffs_[i].second())
        );
    }
...
template<class Type>
Type Function1Types::Polynomial<Type>::integrate
(
    const scalar x1,
    const scalar x2
) const
    ...
    if (canIntegrate_)
    {
        forAll(coeffs_, i)
        {
            ...
            (
                ...
                (
                    coeffs_[i].first(),
                    coeffs_[i].second() + pTraits<Type>::one_
                ),
                cmptPow
                (
                    pTraits<Type>::one_*x2,
                    coeffs_[i].second() + pTraits<Type>::one_
                )
              - cmptPow
                (
                    pTraits<Type>::one_*x1,
                    coeffs_[i].second() + pTraits<Type>::one_
                )
            );
        }
...
template<class Type>
void Function1Types::Polynomial<Type>::writeData(Ostream& os) const
{
