...
namespace Foam {
    ...
    template<>
    const char* const symmTensor::vsType::typeName = "symmTensor";

    template<>
    const char* const symmTensor::vsType::componentNames[] =
    {
        "xx", "xy", "xz",
              "yy", "yz",
                    "zz"
    ...
    template<>
    const symmTensor symmTensor::vsType::vsType::zero_
    (
        symmTensor::uniform(0)
    );

    template<>
    const symmTensor symmTensor::vsType::one_
    (
        symmTensor::uniform(1)
    );

    template<>
    const symmTensor symmTensor::vsType::max_
    (
        symmTensor::uniform(VGREAT)
    );

    template<>
    const symmTensor symmTensor::vsType::min_
    (
        symmTensor::uniform(-VGREAT)
    );

    template<>
    const symmTensor symmTensor::vsType::rootMax_
    (
        symmTensor::uniform(ROOTVGREAT)
    );

    template<>
    const symmTensor symmTensor::vsType::rootMin_
    (
        symmTensor::uniform(-ROOTVGREAT)
    );
