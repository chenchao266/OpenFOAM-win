...
const labelUList& lduAddressing::losortAddr() const
{
    if (!losortPtr_)
    {
        calcLosort();
...
const labelUList& lduAddressing::ownerStartAddr() const
{
    if (!ownerStartPtr_)
    {
        calcOwnerStart();
...
const labelUList& lduAddressing::losortStartAddr() const
{
    if (!losortStartPtr_)
    {
        calcLosortStart();
...
Tuple2<label, scalar> lduAddressing::band() const
{
