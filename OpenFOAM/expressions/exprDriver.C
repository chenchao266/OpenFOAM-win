...
// * * * * * * * * * * * * * Static Member Functions * * * * * * * * * * * * //
 ...
 namespace Foam{
int expressions::exprDriver::getSearchControls(const dictionary& dict)
{
    int val = 0;
...
// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //
 ...
 namespace Foam{
void expressions::exprDriver::resetTimeReference(const TimeState* ts)
{
    timeStatePtr_ = ts;
}
...
void expressions::exprDriver::resetDb(const objectRegistry* obrPtr)
{
    obrPtr_ = obrPtr;
...
void expressions::exprDriver::resetDb(const objectRegistry& db)
{
    resetDb(&db);
}
...
expressions::exprDriver::exprDriver
(
...
const TimeState* expressions::exprDriver::timeState() const noexcept
{
    if (timeStatePtr_)
    {
...
scalar expressions::exprDriver::timeValue() const
{
    if (timeStatePtr_)
    {
...
scalar expressions::exprDriver::deltaT() const
{
    if (timeStatePtr_)
    {
...
bool expressions::exprDriver::readDict
(
    const dictionary& dict
)
{
...
void expressions::exprDriver::clearResult()
{
    result_.clear();
}


bool expressions::exprDriver::update()
{
    return true;
}


void expressions::exprDriver::updateSpecialVariables(bool force)
{}


void expressions::exprDriver::clearVariables()
{
    variables_.clear();
...
void expressions::exprDriver::evaluateVariable
(
    const word& varName,
...
void expressions::exprDriver::evaluateVariableRemote
(
    string remote,
    const word& varName,
    const expressions::exprString& expr
...
expressions::exprDriver::getRemoteResult
(
...
void expressions::exprDriver::addVariables
(
    const expressions::exprString& expr,
    bool clear
)
...
void expressions::exprDriver::setDebugging
(
    bool scannerDebug,
    bool parserDebug
)
...
bool expressions::exprDriver::setCaching(bool on) noexcept
{
    int val(searchCtrl_);
...
void expressions::exprDriver::setSearchBehaviour
(
    enum searchControls search,
    const bool caching
)
