...
void OFstreamCollator::waitForBufferSpace(const offset_t wantedSize) const
{
    while (true)
    {
...
OFstreamCollator::OFstreamCollator(const offset_t maxBufferSize) :    maxBufferSize_(maxBufferSize),
    mutex_
    (
        maxBufferSize_ > 0
      ? allocateMutex()
