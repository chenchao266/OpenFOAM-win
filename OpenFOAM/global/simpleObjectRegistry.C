...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
 ...
 namespace Foam{
void simpleObjectRegistry::setValues
(
...
)
{
    ...
    const bool log = (report && infoDetailLevel > 0);
...
void simpleObjectRegistry::setNamedValue
(
    std::string name,
    int val,
    bool report
)
{
    ...
    if (eq != std::string::npos)
    {
        ...
        if (readInt(strval, val))
        {
            tok = static_cast<label>(val);
        }
        else if (readFloat(strval, fvalue))
        {
            tok = fvalue;
        }
