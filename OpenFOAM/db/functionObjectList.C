...
bool functionObjectList::readFunctionObject
(
...
autoPtr<functionObjectList> functionObjectList::New
(
    const argList& args,
    const Time& runTime,
...
bool functionObjectList::status() const
{
    return execution_;
}


bool functionObjectList::start()
{
    return read();
}
...
bool functionObjectList::adjustTimeStep()
{
    bool ok = true;
...
void functionObjectList::updateMesh(const mapPolyMesh& mpm)
{
    if (execution_)
    {
        for (functionObject& funcObj : functions())
...
void functionObjectList::movePoints(const polyMesh& mesh)
{
    if (execution_)
    {
        for (functionObject& funcObj : functions())
