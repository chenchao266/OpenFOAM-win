...
namespace Foam
{
    ...
    const Enum<functionObjects::timeControl::controlMode>
        functionObjects::timeControl::controlModeNames_
        ({
    ...
    void functionObjects::timeControl::readControls()
    {
    ...
    bool functionObjects::timeControl::active() const
    {
    ...
    scalar functionObjects::timeControl::calcExpansion
    (
    ...
    )
    {
        ...
        for (label iter = 0; iter < 100; iter++)
        {
            ...
            scalar f = (y - 1)*pow(ratio, (const int)n) + 1 - y * pow(ratio, (const int)n - 1);
            scalar dfdratio = (y - 1)*n*pow(ratio, (const int)n - 1) - y * (n - 1)*pow(ratio, (const int)n - 2);
    ...
    void functionObjects::timeControl::calcDeltaTCoeff
    (
    ...
    )
    {
        ...
        if (requiredDeltaTCoeff != 1.0)
        {
            requiredTimeInterval *=
                (pow(requiredDeltaTCoeff, (const int)nSteps) - 1)
        ...
        else
        {
            ...
            for (label iter = 0; iter < 100; iter++)
            {
                ...
                const scalar deltaTLoop =
                    wantedDT
                    / pow(newRatio, (const int)::Foam::mag(requiredSteps) - 1);
    ...
    functionObjects::timeControl::timeControl
    (
        const word& name,
        const Time& runTime,
    ...
    bool functionObjects::timeControl::entriesPresent(const dictionary& dict)
    {
        if
            (
    ...
    bool functionObjects::timeControl::execute()
    {
    ...
    bool functionObjects::timeControl::execute(const label subIndex)
    {
        if (active())
        {
    ...
    bool functionObjects::timeControl::write()
    {
    ...
    bool functionObjects::timeControl::end()
    {
    ...
    bool functionObjects::timeControl::adjustTimeStep()
    {
        if (active())
        {
            if
    ...
    bool functionObjects::timeControl::read(const dictionary& dict)
    {
        if (dict != dict_)
        {
            dict_ = dict;
    ...
    bool functionObjects::timeControl::filesModified() const
    {
        return active() ? foPtr_->filesModified() : false;
    }


    void functionObjects::timeControl::updateMesh(const mapPolyMesh& mpm)
    {
        if (active())
        {
    ...
    void functionObjects::timeControl::movePoints(const polyMesh& mesh)
    {
        if (active())
        {
