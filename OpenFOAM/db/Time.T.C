...
const NamedEnum<Time::stopAtControls, 4>
    Time::stopAtControlNames_;

const NamedEnum<Time::writeControls, 5>
    Time::writeControlNames_;

Time::fmtflags Time::format_(Time::general);

int Time::precision_(6);

const int Time::maxPrecision_(3 - ::Foam::log10(SMALL));
...
instant Time::findClosestTime(const scalar t) const
{
    instantList timeDirs = findTimes(path(), constant());
...
label Time::findClosestTimeIndex
(
...
bool Time::stopAt(const stopAtControls sa) const
{
...
void Time::setDeltaT(const scalar deltaT, const bool bAdjustDeltaT)
{
    deltaT_ = deltaT;
    deltaTchanged_ = true;
