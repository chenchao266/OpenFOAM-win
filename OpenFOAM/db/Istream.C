...
inline std::streampos stream_tellg(Istream* isptr)
{
    auto* sptr = dynamic_cast<ISstream*>(isptr);
...
const token& Istream::peekBack() const noexcept
{
    return (putBackAvail_ ? *putBackToken_ : token::undefinedToken);
}


bool Istream::peekBack(token& tok)
{
    if (putBackAvail_)
    {
        tok = *putBackToken_;
...
void Istream::putBack(const token& tok)
{
    if (bad())
    {
...
bool Istream::getBack(token& tok)
{
    if (bad())
    {
        FatalIOErrorInFunction(*this)
...
bool Istream::readBegin(const char* funcName)
{
...
bool Istream::readEnd(const char* funcName)
{
    const token delimiter(*this);
