...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
 ...
 namespace Foam{
bool OSstream::write(const token& tok)
{
...
Ostream& OSstream::write(const char* str)
{
...
Ostream& OSstream::write(const word& str)
{
    os_ << str;
    setState(os_.rdstate());
    return *this;
...
Ostream& OSstream::write(const string& str)
{
    return writeQuoted(str, true);
}


Ostream& OSstream::write(const int32_t val)
{
    os_ << val;
    setState(os_.rdstate());
    return *this;
...
Ostream& OSstream::write(const int64_t val)
{
    os_ << val;
    setState(os_.rdstate());
    return *this;
...
Ostream& OSstream::write(const floatScalar val)
{
    os_ << val;
    setState(os_.rdstate());
    return *this;
...
Ostream& OSstream::write(const doubleScalar val)
{
    os_ << val;
    setState(os_.rdstate());
    return *this;
...
Ostream& OSstream::write(const char* data, std::streamsize count)
{
    beginRawWrite(count);
    writeRaw(data, count);
    endRawWrite();
...
bool OSstream::beginRawWrite(std::streamsize count)
{
    if (format() != BINARY)
    {
...
std::ios_base::fmtflags OSstream::flags() const
{
    return os_.flags();
}


std::ios_base::fmtflags OSstream::flags(const ios_base::fmtflags f)
{
    return os_.flags(f);
}
...
char OSstream::fill() const
{
    return os_.fill();
}


char OSstream::fill(const char fillch)
{
