...
bool ISstream::seekCommentEnd_Cstyle()
{
...
// * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * * //
 ...
 namespace Foam{
bool ISstream::continueReadUntilRightBrace
(
    std::string& str,
    const bool stripComments
)
    ...
    switch (c)
    {
        ...
        case '5' : case '6' : case '7' : case '8' : case '9' :
        {
            ...
            else
            {
                ...
                else if (labelVal && read(buf, labelVal))
                {
                    t = labelVal;
                }
                else
...
Istream& ISstream::read(word& str)
{
    constexpr const unsigned bufLen = 1024;
    static char buf[bufLen];
...
Istream& ISstream::read(string& str)
{
    constexpr const unsigned bufLen = 1024;
    static char buf[bufLen];
...
Istream& ISstream::read(label& val)
{
    is_ >> val;
    setState(is_.rdstate());
    return *this;
...
Istream& ISstream::read(floatScalar& val)
{
    is_ >> val;
    setState(is_.rdstate());
    return *this;
...
Istream& ISstream::read(doubleScalar& val)
{
    is_ >> val;
    setState(is_.rdstate());
    return *this;
...
Istream& ISstream::readRaw(char* buf, std::streamsize count)
{
    is_.read(buf, count);
    setState(is_.rdstate());
...
std::ios_base::fmtflags ISstream::flags() const
{
    return is_.flags();
}


std::ios_base::fmtflags ISstream::flags(const ios_base::fmtflags f)
{
    return is_.flags(f);
}
