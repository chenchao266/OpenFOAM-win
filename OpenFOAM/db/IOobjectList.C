...
// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //
 ...
 namespace Foam{
bool IOobjectList::checkNames(wordList& masterNames, const bool syncPar)
{
...
void IOobjectList::syncNames(wordList& objNames)
{
    if (Pstream::parRun())
    {
        // Synchronize names
...
IOobjectList::IOobjectList(const label nObjects)
:
    HashPtrTable<IOobject>(nObjects)  // Could also use 2*nObjects instead
{}


IOobjectList::IOobjectList(const IOobjectList& list)
:
    HashPtrTable<IOobject>(list)
{}


IOobjectList::IOobjectList(IOobjectList&& list)
:
...
bool IOobjectList::add(autoPtr<IOobject>& objectPtr)
{
    if (objectPtr)
    {
...
label IOobjectList::append(const IOobjectList& other)
{
    label count = 0;
...
label IOobjectList::append(IOobjectList&& other)
{
...
bool IOobjectList::remove(const IOobject& io)
{
...
IOobjectList IOobjectList::lookupClass(const char* clsName) const
{
...
HashTable<wordHashSet> IOobjectList::classes() const
{
    return classesImpl(*this, predicates::always());
}


label IOobjectList::count(const char* clsName) const
{
    // No nullptr check - only called with string literals
...
wordList IOobjectList::names(const char* clsName) const
{
    // No nullptr check - only called with string literals
...
wordList IOobjectList::sortedNames(const char* clsName) const
{
    // No nullptr check - only called with string literals
...
bool IOobjectList::checkNames(const bool syncPar) const
{
    if (syncPar && Pstream::parRun())
    {
        wordList objNames(HashPtrTable<IOobject>::toc());
...
void IOobjectList::operator=(IOobjectList&& list)
{
    transfer(list);
}
