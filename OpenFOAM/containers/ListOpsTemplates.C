...
)
{
    ...
    stableSort(order, comp);
}


template<class T>
labelList duplicateOrder
(
    const UList<T>& input
)
{
...
template<class ListType>
ListType rotateList(const ListType& input, const label n)
{
    const label len = input.size();
...
void inplaceRotateList(ListType<DataType>& input, label n)
{
    const label len = input.size();
...
template<class T>
void ListOps::appendEqOp<T>::operator()
(
    List<T>& x,
    const List<T>& y
) const
...
template<class T>
void ListOps::uniqueEqOp<T>::operator()
(
    List<T>& x,
    const List<T>& y
) const
...
template<class T>
List<T> ListOps::createWithValue
(
    const label len,
    const labelUList& locations,
    const T& val,
