...
template<unsigned Width>
bool PackedList<Width>::uniform() const
{
    // Trivial cases
    if (empty())
    {
...
template<unsigned Width>
bool PackedList<Width>::equal(const PackedList<Width>& other) const
{
    if (size() != other.size())
    {
        return false;
...
template<unsigned Width>
labelList PackedList<Width>::values() const
{
...
template<class IntType>
List<IntType>
PackedList<Width>::unpack(const labelRange& range) const
{
    static_assert
    (
        std::is_integral<IntType>::value,
...
template<class IntType>
List<IntType>
PackedList<Width>::unpack(const labelUList& locations) const
{
    static_assert
    (
        std::is_integral<IntType>::value,
