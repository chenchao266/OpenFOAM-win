  ...
  \\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox
   \\    /   O peration     |
    \\  /    A nd           | Copyright (C) 2011-2016 OpenFOAM Foundation
     \\/     M anipulation  |
-------------------------------------------------------------------------------
 2011 Symscape: Added "inline" to "canonicalSize".
...
namespace Foam {
    inline
        label StaticHashTableCore::canonicalSize(const label size)
    {
        if (size < 1)
        {
            return 0;
    ...
    StaticHashTable<T, Key, Hash>::StaticHashTable(const label size) : StaticHashTableCore(),
    ...
    template<class T, class Key, class Hash>
    bool StaticHashTable<T, Key, Hash>::set
    (
        const Key& key,
        const T& newEntry,
        const bool protect
    ...
    template<class T, class Key, class Hash>
    bool StaticHashTable<T, Key, Hash>::erase(const iterator& cit)
    {
        if (cit != end())
        {
    ...
    template<class T, class Key, class Hash>
    label StaticHashTable<T, Key, Hash>::erase
    (
    ...
    template<class T, class Key, class Hash>
    void StaticHashTable<T, Key, Hash>::resize(const label sz)
    {
    ...
    template<class T, class Key, class Hash>
    void StaticHashTable<T, Key, Hash>::clearStorage()
    {
        clear();
        resize(1);
    }
    ...
    template<class T, class Key, class Hash>
    void StaticHashTable<T, Key, Hash>::transfer
    (
    ...
    template<class T, class Key, class Hash>
    bool StaticHashTable<T, Key, Hash>::operator==
        (
            const StaticHashTable<T, Key, Hash>& rhs
            ) const
    {
