...
namespace Foam {
    ...
    template<class ListType>
    label findMax(const ListType& l, const label start)
    {
        if (start >= l.size())
        {
            return -1;
    ...
    template<class ListType>
    label findMin(const ListType& l, const label start)
    {
        if (start >= l.size())
        {
            return -1;
    ...
    List<Container> initList(const T elems[mRows])
    {
    ...
    List<Container> initListList(const T elems[mRows][nColumns])
    {
        List<Container> lst(mRows);
    ...
    template<class T>
    void ListAppendEqOp<T>::operator()(List<T>& x, const List<T>& y) const
    {
        if (y.size())
        {
            if (x.size())
