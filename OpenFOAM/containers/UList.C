...
template<class T>
labelRange
UList<T>::validateRange(const labelRange& requestedRange) const
{
...
template<class T>
void UList<T>::moveFirst(const label i)
{
    checkIndex(i);

    for (label lower = 0; lower < i; ++lower)
    {
        Swap(this->operator[](lower), this->operator[](i));
    }
}


template<class T>
void UList<T>::moveLast(const label i)
{
    checkIndex(i);

    for (label upper = size()-1; upper > i; --upper)
    {
        Swap(this->operator[](i), this->operator[](upper));
    }
}


template<class T>
void UList<T>::swapFirst(const label i)
{
    checkIndex(i);

    if (i > 0)
    {
        Swap(this->operator[](0), this->operator[](i));
    }
}


template<class T>
void UList<T>::swapLast(const label i)
{
    checkIndex(i);
...
template<class T>
void UList<T>::deepCopy(const UList<T>& list)
{
...
template<class T>
template<class Addr>
void UList<T>::deepCopy(const IndirectListBase<T, Addr>& list)
{
    const label len = this->size_;
...
template<class T>
void UList<T>::operator=(const T& val)
{
...
template<class T>
void UList<T>::operator=(const zero)
{
    const label len = this->size();
...
template<class T>
std::streamsize UList<T>::byteSize() const
{
    if (!is_contiguous<T>::value)
    {
        FatalErrorInFunction
...
template<class T>
label UList<T>::find(const T& val, label pos) const
{
    const label len = this->size();
...
template<class T>
label UList<T>::rfind(const T& val, label pos) const
{
...
template<class T>
bool UList<T>::operator==(const UList<T>& list) const
{
    const label len = this->size_;
    if (len != list.size_)
    {
