...
bool PackedBoolList::bitorPrepare
(
...
label PackedBoolList::setIndices(const LabelListType& indices)
{
...
template<class LabelListType>
label PackedBoolList::unsetIndices(const LabelListType& indices)
{
    label cnt = 0;
    forAll(indices, elemI)
    {
...
template<class LabelListType>
label PackedBoolList::subsetIndices(const LabelListType& indices)
{
    // handle trivial case
...
PackedBoolList::PackedBoolList(Istream& is) :    PackedList<1>()
{
    is  >> *this;
}
...
void PackedBoolList::set(const PackedList<1>& lst)
{
...
label PackedBoolList::set(const labelUList& indices)
{
    return setIndices(indices);
}


label PackedBoolList::set(const UIndirectList<label>& indices)
{
    return setIndices(indices);
}


void PackedBoolList::unset(const PackedList<1>& lst)
{
    // operate directly with the underlying storage
    StorageList& lhs = this->storage();
    const StorageList& rhs = lst.storage();
...
label PackedBoolList::unset(const labelUList& indices)
{
    return unsetIndices(indices);
}


label PackedBoolList::unset(const UIndirectList<label>& indices)
{
    return unsetIndices(indices);
}


void PackedBoolList::subset(const PackedList<1>& lst)
{
...
label PackedBoolList::subset(const labelUList& indices)
{
    return subsetIndices(indices);
}


label PackedBoolList::subset(const UIndirectList<label>& indices)
{
    return subsetIndices(indices);
}


Xfer<labelList> PackedBoolList::used() const
{
...
void PackedBoolList::operator=(const UList<bool>& lst)
{
