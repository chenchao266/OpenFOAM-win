...
)
{
    ...
    if (this->cyclicAMIPatch().owner())
    {
        ...
        if (matrix.psi(mat).mesh().fluxRequired(this->internalField().name()))
        {
            ...
            (
                globalPatchID, intCoeffs*pTraits<Type>::one_
            );
            matrix.boundaryCoeffs().set
            (
                globalPatchID, bndCoeffs*pTraits<Type>::one_
            );
            ...
            matrix.internalCoeffs().set
            (
                nbrGlobalPatchID, intCoeffs*pTraits<Type>::one_
            );
            matrix.boundaryCoeffs().set
            (
                nbrGlobalPatchID, bndCoeffs*pTraits<Type>::one_
            );
        }
    }
}
