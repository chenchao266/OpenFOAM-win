...
{
    const GeometricField<Type, Foam::fvPatchField, volMesh>& fld =
...
)
{
    if (this->cyclicPatch().owner())
    {
        ...
        if (matrix.psi(mat).mesh().fluxRequired(this->internalField().name()))
        {
            ...
            (
                globalPatchID, intCoeffsCmpt*pTraits<Type>::one_
            );
            matrix.boundaryCoeffs().set
            (
                globalPatchID, boundCoeffsCmpt*pTraits<Type>::one_
            );
            ...
            matrix.internalCoeffs().set
            (
                nbrGlobalPatchID, intCoeffsCmpt*pTraits<Type>::one_

            );
            matrix.boundaryCoeffs().set
            (
                nbrGlobalPatchID, boundCoeffsCmpt*pTraits<Type>::one_
            );
        }
    }
}
