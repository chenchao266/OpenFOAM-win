    ...
    for (label patchi = 0; patchi < nOldInterfaces; ++patchi)
    {
        ...
        forAll(fc, i)
        {
            label celli = fc[i];
            {
                ...
                {
                    intCoeffs[i] = pTraits<Type>::zero_;
                    bouCoeffs[i] = pTraits<Type>::zero_;
                }
            }
        }
    }
        ...
        else
        {
            ...
            forAll(nbrs, nbri)
            {
                ...
                else
                {
                    ...
                    const scalar s = normalisation[celli]*f*w[nbri];
                    m.boundaryCoeffs()[patchi][facei] += pTraits<Type>::one_*s;
                    m.internalCoeffs()[patchi][facei] += pTraits<Type>::one_*s;
